...
export function DataStatusBar() {
  ...
  const statusLabel =
    LIVE_META.status === "stale"
      ? `데이터 ${LIVE_META.delayTradingDays}거래일 지연`
      : status.label;
  const StatusIcon =
    LIVE_META.status === "normal" ? CheckCircle2 : AlertTriangle;
  const warning =
    LIVE_META.status === "stale"
      ? `${formatMarketDate(LIVE_META.asOf)} 기준 데이터입니다. 최신 미국 거래일보다 ${LIVE_META.delayTradingDays}거래일 늦어 갱신 전 수치로 표시합니다.`
  ...
  return (
    ...
    >
      <div className="flex flex-wrap items-center gap-x-4 gap-y-2 text-muted-foreground">
        <span className="inline-flex items-center gap-1.5">
          ...
          <strong className="font-semibold text-foreground">
            {formatMarketDate(LIVE_META.asOf)} 미국 정규장 종가
          </strong>
        </span>
        <span className="inline-flex items-center gap-1.5">
        ...
        <a
          href="/methodology/"
          className="ml-auto inline-flex min-h-8 items-center gap-1.5 font-semibold text-brand hover:underline sm:min-h-0"
        >
      ...
      {warning && (
        <p className="mt-2 border-t border-warning/20 pt-2 font-medium text-warning">
          {warning}
        </p>
      )}
    </div>
