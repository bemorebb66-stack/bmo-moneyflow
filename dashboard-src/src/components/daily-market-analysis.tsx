...
const shareOf = (
  rows: StockRow[],
  count: number,
  value: (stock: StockRow) => number | null,
) => {
  const valid = rows
    .map((stock) => ({ stock, value: value(stock) }))
    .filter(
      (item): item is { stock: StockRow; value: number } =>
...
const groupShare = (
  ...
  const selected = valid
    .filter((item) => tickers.has(item.stock.ticker))
    .reduce((sum, item) => sum + item.value, 0);
...
export function DailyMarketAnalysis() {
  const analysis = useMemo(() => {
    const valid = LIVE_STOCKS.filter(
      (stock) => Number.isFinite(stock.volumeRatio) && stock.volumeRatio > 0,
    );
    ...
    const upPriceDownValue = valid.filter(
      (stock) => stock.change > 0 && stock.volumeRatio < 1,
    );
    const downPriceDownValue = valid.filter(
      (stock) => stock.change <= 0 && stock.volumeRatio < 1,
    );
    const participation = valid.length
      ? (upValue.length / valid.length) * 100
      : 0;
    ...
    concentration.push({
      ...
      average: groupShare(valid, MEGA_CAP_7, (stock) =>
        averageVolume(stock, "5d"),
      ),
    });
  ...
  useEffect(() => {
    ...
    track("concentration_view", {
      section_name: "trading_value_concentration",
    });
  }, []);
  ...
  return (
    <section className="mt-5" aria-label="오늘의 시장 상세 분석">
      <div className="grid grid-cols-1 gap-5 lg:grid-cols-12">
        <QuadrantCard
          analysis={analysis}
          className="order-4 lg:order-1 lg:col-span-5"
        />
        <ParticipationCard
          analysis={analysis}
...
function ParticipationCard({
  ...
  const data = [
    {
      name: "상승 + 거래대금 증가",
      value: analysis.upPriceUpValue.length,
      color: "var(--color-success)",
    },
    {
      name: "하락 + 거래대금 증가",
      value: analysis.downPriceUpValue.length,
      color: "var(--color-danger)",
    },
    {
      name: "거래대금 감소",
      value: analysis.valid.length - analysis.upValue.length,
  ...
  return (
    <Card className={className}>
      <CardContent className="p-4 sm:p-5">
        ...
        />
        <div
          className="relative mt-3 h-44"
          role="img"
          aria-label={`시장 참여도 ${analysis.participation.toFixed(0)}%. 거래대금 증가 종목 ${analysis.upValue.length}개, 분석 대상 ${analysis.valid.length}개`}
        >
          ...
          <div className="pointer-events-none absolute inset-0 grid place-content-center text-center">
            <strong className="text-2xl tabular">
              {analysis.participation.toFixed(0)}%
            </strong>
            <span className="text-[10px] text-muted-foreground">
              거래대금 증가
            </span>
            <span className="text-[10px] tabular text-muted-foreground">
              {analysis.upValue.length} / {analysis.valid.length}
            </span>
          </div>
        </div>
          ...
          {data.map((item) => (
            <div key={item.name} className="flex items-center gap-2">
              ...
              <span className="font-medium tabular">
                {item.value}종목 ·{" "}
                {((item.value / analysis.valid.length) * 100).toFixed(0)}%
              </span>
            </div>
          ))}
        </div>
        <p className="mt-3 rounded-md bg-surface-2 px-3 py-2 text-xs font-medium">
          {message}
        </p>
      </CardContent>
    </Card>
        ...
        <div className="mt-4 divide-y divide-border/70">
          {analysis.concentration.map((item) => {
            ...
            const displayedDifference =
              Math.abs(difference) < 0.05 ? 0 : difference;
            return (
              <div key={item.label} className="py-2.5 first:pt-0">
                <div className="flex items-baseline justify-between gap-3">
                  ...
                  <strong className="text-base tabular">
                ...
                <div className="mt-0.5 flex justify-between text-[11px] tabular text-muted-foreground">
                  ...
                  >
                    {displayedDifference > 0 ? "+" : ""}
                    {displayedDifference.toFixed(1)}%p
                  </span>
                </div>
              </div>
            );
...
function QuadrantCard({
  ...
  const openStock = (point: QuadrantPoint) => {
    track("quadrant_point_click", {
      ticker: point.ticker,
      section_name: "price_value_quadrant",
    });
    ...
    <Card className={className}>
      <CardContent className="p-4 sm:p-5">
        ...
        <Link
          to="/scanner"
          search={
            { priceDirection: "up", tradingValueDirection: "up" } as never
          }
          onClick={() =>
  ...
  return (
    <div className="rounded-lg border border-border bg-popover p-3 text-xs shadow-xl">
      ...
      <div className="grid grid-cols-[auto_auto] gap-x-4 gap-y-1 tabular">
        <span className="text-muted-foreground">주가 변화</span>
        <span>{fmtPct(stock.change)}</span>
        <span className="text-muted-foreground">현재 거래대금</span>
        <span>{fmtMoney(stock.volume)}</span>
        <span className="text-muted-foreground">20일 평균 대비</span>
        <span>{stock.volumeRatio.toFixed(2)}배</span>
            ...
            <tbody className="divide-y divide-border/70">
              {shown.map((stock, index) => (
                <tr key={stock.ticker} className="hover:bg-secondary/45">
                  ...
                  <td className="px-3 py-3 text-right font-semibold tabular text-brand">
                    {stock.volumeRatio.toFixed(2)}배
                  </td>
                  <td
                    className={cn(
                      "px-3 py-3 text-right tabular",
                      tone(stock.volumeVs?.["1d"] ?? 0),
                    )}
                  >
                    {fmtPct(stock.volumeVs?.["1d"] ?? 0, 1)}
                  </td>
                  <td
                    className={cn(
...
function SectorChangeCard({ className }: { className?: string }) {
  const all = [...SECTORS].sort(
    (a, b) => Math.abs(b.shareDelta) - Math.abs(a.shareDelta),
  );
  const mobile = [
    ...SECTORS.filter((sector) => sector.shareDelta > 0)
  ...
  const chart = (data: typeof SECTORS, height: number) => (
    <ResponsiveContainer width="100%" height={height}>
      <BarChart
        data={data}
        layout="vertical"
        margin={{ top: 4, right: 44, bottom: 4, left: 10 }}
      >
        <CartesianGrid
          horizontal={false}
          stroke="var(--color-border)"
        ...
        <XAxis
          type="number"
          tickFormatter={(value) => `${(value / 100).toFixed(1)}%p`}
          fontSize={9}
          tickLine={false}
        />
        <YAxis
        ...
        <Bar
          ...
          radius={2}
          onClick={() =>
            track("sector_chart_interaction", {
              section_name: "sector_share_change",
            })
          }
        >
          {data.map((sector) => (
