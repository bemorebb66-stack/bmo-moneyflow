export const INDUSTRY_KO: Record<string, string> = {
  "Advertising Agencies": "광고대행",
  "Aerospace & Defense": "항공우주·방산",
  "Agricultural Inputs": "농업 투입재",
  Airlines: "항공사",
  "Airports & Air Services": "공항·항공 서비스",
  Aluminum: "알루미늄",
  "Apparel Manufacturing": "의류 제조",
  "Apparel Retail": "의류 소매",
  "Asset Management": "자산운용",
  "Auto & Truck Dealerships": "자동차 판매",
  "Auto Manufacturers": "자동차 제조",
  "Auto Parts": "자동차 부품",
  "Banks - Diversified": "은행(대형)",
  "Banks - Regional": "은행(지역)",
  "Beverages - Brewers": "주류(맥주)",
  "Beverages - Non-Alcoholic": "비알코올 음료",
  "Beverages - Wineries & Distilleries": "와인·증류주",
  Biotechnology: "바이오",
  Broadcasting: "방송",
  "Building Materials": "건축 소재",
  "Building Products & Equipment": "건축자재·설비",
  "Capital Markets": "증권·IB",
  Chemicals: "화학",
  "Coking Coal": "제철용 석탄",
  "Communication Equipment": "통신장비",
  "Computer Hardware": "컴퓨터 하드웨어",
  Confectioners: "제과",
  Conglomerates: "복합기업",
  "Consulting Services": "컨설팅",
  "Consumer Electronics": "가전·전자기기",
  Copper: "구리",
  "Credit Services": "신용·결제",
  "Department Stores": "백화점",
  "Diagnostics & Research": "진단·리서치",
  "Discount Stores": "할인점",
  "Drug Manufacturers - General": "제약(대형)",
  "Drug Manufacturers - Specialty & Generic": "제약(전문·제네릭)",
  "Education & Training Services": "교육·훈련",
  "Electrical Equipment & Parts": "전기장비",
  "Electronic Components": "전자부품",
  "Electronic Gaming & Multimedia": "게임·멀티미디어",
  "Electronics & Computer Distribution": "전자·컴퓨터 유통",
  "Engineering & Construction": "건설·엔지니어링",
  Entertainment: "엔터테인먼트",
  "Farm & Heavy Construction Machinery": "농업·중장비",
  "Farm Products": "농산물",
  "Financial Conglomerates": "금융 복합기업",
  "Financial Data & Stock Exchanges": "금융 데이터·거래소",
  "Food Distribution": "식품 유통",
  "Footwear & Accessories": "신발·액세서리",
  "Furnishings, Fixtures & Appliances": "가구·가전",
  Gambling: "도박·베팅",
  Gold: "금",
  "Grocery Stores": "식료품점",
  "Health Information Services": "헬스케어 정보 서비스",
  "Healthcare Plans": "건강보험",
  "Home Improvement Retail": "주택개량 소매",
  "Household & Personal Products": "생활·개인용품",
  "Industrial Distribution": "산업재 유통",
  "Information Technology Services": "IT 서비스",
  "Insurance - Diversified": "보험(종합)",
  "Insurance - Life": "생명보험",
  "Insurance - Property & Casualty": "손해보험",
  "Insurance - Reinsurance": "재보험",
  "Insurance - Specialty": "전문보험",
  "Insurance Brokers": "보험 중개",
  "Integrated Freight & Logistics": "물류",
  "Internet Content & Information": "인터넷 콘텐츠·정보",
  "Internet Retail": "인터넷 소매",
  Leisure: "레저",
  Lodging: "숙박",
  "Lumber & Wood Production": "목재 생산",
  "Luxury Goods": "명품",
  "Marine Shipping": "해운",
  "Medical Care Facilities": "의료시설",
  "Medical Devices": "의료기기",
  "Medical Distribution": "의료 유통",
  "Medical Instruments & Supplies": "의료 장비·소모품",
  "Metal Fabrication": "금속 가공",
  "Mortgage Finance": "주택담보 금융",
  "Oil & Gas Drilling": "석유·가스 시추",
  "Oil & Gas E&P": "석유·가스 탐사·생산",
  "Oil & Gas Equipment & Services": "석유·가스 장비·서비스",
  "Oil & Gas Integrated": "석유·가스(통합)",
  "Oil & Gas Midstream": "석유·가스 미드스트림",
  "Oil & Gas Refining & Marketing": "정유·판매",
  "Other Industrial Metals & Mining": "산업용 금속·광업",
  "Other Precious Metals & Mining": "귀금속·광업",
  "Packaged Foods": "가공식품",
  "Packaging & Containers": "포장재·용기",
  "Personal Services": "개인 서비스",
  "Pollution & Treatment Controls": "환경오염·처리 설비",
  Publishing: "출판",
  Railroads: "철도",
  "Real Estate - Development": "부동산 개발",
  "Real Estate - Diversified": "부동산(종합)",
  "Real Estate Services": "부동산 서비스",
  "Recreational Vehicles": "레저용 차량",
  "REIT - Diversified": "리츠(종합)",
  "REIT - Healthcare Facilities": "리츠(의료시설)",
  "REIT - Hotel & Motel": "리츠(호텔)",
  "REIT - Industrial": "리츠(물류·산업)",
  "REIT - Mortgage": "리츠(모기지)",
  "REIT - Office": "리츠(오피스)",
  "REIT - Residential": "리츠(주거)",
  "REIT - Retail": "리츠(리테일)",
  "REIT - Specialty": "리츠(특수자산)",
  "Rental & Leasing Services": "임대·리스",
  "Residential Construction": "주택 건설",
  "Resorts & Casinos": "리조트·카지노",
  Restaurants: "외식",
  "Scientific & Technical Instruments": "과학·기술 장비",
  "Security & Protection Services": "보안·안전 서비스",
  "Semiconductor Equipment & Materials": "반도체 장비·소재",
  Semiconductors: "반도체",
  "Software - Application": "소프트웨어(응용)",
  "Software - Infrastructure": "소프트웨어(인프라)",
  Solar: "태양광",
  "Specialty Business Services": "기업 전문 서비스",
  "Specialty Chemicals": "특수화학",
  "Specialty Industrial Machinery": "산업기계",
  "Specialty Retail": "전문 소매",
  "Staffing & Employment Services": "인력·고용 서비스",
  Steel: "철강",
  "Telecom Services": "통신 서비스",
  "Textile Manufacturing": "섬유 제조",
  "Thermal Coal": "발전용 석탄",
  Tobacco: "담배",
  "Tools & Accessories": "공구·부품",
  "Travel Services": "여행",
  Trucking: "트럭 운송",
  Uranium: "우라늄",
  "Utilities - Diversified": "유틸리티(종합)",
  "Utilities - Independent Power Producers": "독립 발전",
  "Utilities - Regulated Electric": "전력(규제)",
  "Utilities - Regulated Gas": "가스(규제)",
  "Utilities - Regulated Water": "수도(규제)",
  "Utilities - Renewable": "신재생에너지",
  "Waste Management": "폐기물 관리",
  기타: "기타",
};

const DESCRIPTION_RULES: Array<[RegExp, string]> = [
  [
    /리츠/,
    "부동산을 보유·임대하거나 관련 금융으로 수익을 내는 리츠 기업군입니다.",
  ],
  [
    /은행/,
    "예금·대출과 기업금융을 제공해 이자와 수수료 수익을 내는 은행 기업군입니다.",
  ],
  [/보험/, "보험료를 받고 위험을 보장하며 자산을 운용하는 보험 기업군입니다."],
  [/반도체 장비/, "반도체 생산에 필요한 장비와 소재를 공급하는 기업군입니다."],
  [/반도체/, "칩을 설계·제조하거나 관련 기술을 공급하는 기업군입니다."],
  [
    /소프트웨어/,
    "기업과 소비자가 사용하는 소프트웨어와 클라우드 서비스를 제공하는 기업군입니다.",
  ],
  [/바이오|제약/, "신약과 치료 기술을 연구·개발하고 판매하는 기업군입니다."],
  [
    /의료기기|의료 장비/,
    "진단과 치료에 쓰이는 의료 장비와 소모품을 만드는 기업군입니다.",
  ],
  [
    /석유·가스|정유/,
    "석유·가스를 개발·운송·정제하거나 관련 서비스를 제공하는 기업군입니다.",
  ],
  [
    /전력|가스\(규제\)|수도|발전|신재생|태양광/,
    "전력·가스·수도 또는 신재생에너지 인프라를 운영하는 기업군입니다.",
  ],
  [/자동차/, "자동차를 제조·판매하거나 핵심 부품을 공급하는 기업군입니다."],
  [
    /항공|우주|방산/,
    "항공기·우주 시스템·방위 장비와 관련 서비스를 제공하는 기업군입니다.",
  ],
  [
    /철도|운송|물류|해운|트럭/,
    "사람과 화물을 운송하고 물류망을 운영하는 기업군입니다.",
  ],
  [
    /인터넷|게임|방송|엔터테인먼트|광고|출판/,
    "콘텐츠와 디지털 플랫폼을 통해 이용자와 광고주를 연결하는 기업군입니다.",
  ],
  [
    /소매|백화점|할인점|식료품점|외식/,
    "소비자에게 상품이나 음식·서비스를 직접 판매하는 기업군입니다.",
  ],
  [
    /금|구리|알루미늄|철강|석탄|우라늄|금속|광업/,
    "원자재를 채굴·가공해 산업과 에너지 시장에 공급하는 기업군입니다.",
  ],
  [
    /건축|건설|주택/,
    "건물과 주택을 짓거나 필요한 자재·설비를 공급하는 기업군입니다.",
  ],
  [
    /자산운용|증권|금융 데이터|신용·결제|주택담보 금융/,
    "투자·결제·시장 정보 등 금융 서비스를 제공하는 기업군입니다.",
  ],
];

export function translateIndustry(name?: string) {
  if (!name) return "기타";
  return INDUSTRY_KO[name] || name;
}

export function describeIndustry(name?: string) {
  const translated = translateIndustry(name);
  const matched = DESCRIPTION_RULES.find(([pattern]) =>
    pattern.test(translated),
  );
  return (
    matched?.[1] ??
    `${translated} 분야의 제품과 서비스를 제공하는 기업군입니다.`
  );
}
