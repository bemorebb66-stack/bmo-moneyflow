...
}[] = [
  {
    ...
    children: (
      <div className="space-y-3">
        ...
        <Formula
          label="거래대금 이동평균"
          value="최근 5개 또는 20개 정규 거래일의 일별 거래대금 단순평균"
        />
        <Formula
          label="거래대금 모멘텀 점수"
          value="최신 20일 평균 대비 강도 50점 + 5일/20일 평균 추세 30점 + 최근 5일 지속성 20점"
        />
        <p>
          1일은 직전 거래일, 5일·20일·60일은 당일을 제외한 직전 거래일 평균과
          비교합니다. 주가 등락률은 주식분할과 배당 영향을 줄이기 위해
          조정종가를 사용합니다. 20일선 돌파는 직전 거래일 거래대금이 당시 20일
          평균 이하이고 최신 거래일 거래대금이 최신 20일 평균을 초과한 경우에만
          표시합니다.
        </p>
      </div>
    ),
...
function Formula({ label, value }: { label: string; value: string }) {
  return (
    <div className="grid gap-1 rounded-md bg-surface-2 px-3 py-2.5 sm:grid-cols-[150px_1fr] sm:gap-4">
      <strong className="text-foreground">{label}</strong>
      <span className="font-mono text-xs tabular sm:text-sm">{value}</span>
    </div>
  );
}
