...
function ResultPanel({ trades, warnings, coverage }: { trades: CompletedTrade[]; warnings: string[]; coverage: { first: string | null; last: string | null } }) {
  ...
  return <div className="space-y-5">
    <section className="grid grid-cols-2 gap-3 lg:grid-cols-5"><Metric label="완결 거래" value={`${trades.length}건`} /><Metric label="승률" value={`${winRate.toFixed(1)}%`} /><Metric label="평균 수익률" value={`${avgReturn >= 0 ? "+" : ""}${avgReturn.toFixed(2)}%`} tone={avgReturn >= 0 ? "positive" : "negative"} /><Metric label="평균 보유" value={`${avgHolding.toFixed(1)}일`} /><Metric label="시장환경 연결률" value={`${connectionRate.toFixed(0)}% · ${linked.length}/${trades.length}건`} /></section>
    <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-6"><Metric label="전체 환경 연결" value={`${fullLinked}건`} /><Metric label="기초자산 연결" value={`${underlyingLinked}건`} /><Metric label="섹터·지수 연결" value={`${groupLinked}건`} /><Metric label="상품 데이터만" value={`${productOnly}건`} /><Metric label="상품 매핑 필요" value={`${mappingRequired}건`} /><Metric label="과거 데이터 부족" value={`${historicalMissing}건`} /></div>
    {linked.length < trades.length && <div className="rounded-lg border border-warning/30 bg-warning/5 px-4 py-3 text-sm leading-6 text-warning"><strong>{trades.length}건의 수익률·승률·보유기간은 모두 정상 계산됐습니다.</strong><br />이 중 {linked.length}건은 시장환경과 연결됐으며, 나머지 거래는 종목 매핑 또는 과거 데이터가 부족해 기본 복기만 제공합니다.<div className="mt-1 text-xs opacity-80">시장환경 지원 범위: {coverage.first ?? "-"}~{coverage.last ?? "-"}</div></div>}
