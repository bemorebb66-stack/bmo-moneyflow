...
function ResultPanel({ trades, warnings, coverage }: { trades: CompletedTrade[]; warnings: string[]; coverage: { first: string | null; last: string | null } }) {
  ...
  const excludedPatterns = patterns.filter((row) => row.trades < 5).length;
  const profitable = patterns.filter((row) => row.trades >= 5 && row.averageReturn > 0).sort((a, b) => b.averageReturn - a.averageReturn).slice(0, 5);
  const losing = patterns.filter((row) => row.trades >= 5 && row.averageReturn < 0).sort((a, b) => a.averageReturn - b.averageReturn).slice(0, 5);
  const unlinked = trades.filter((trade) => !linked.includes(trade));
  return <div className="space-y-5">
    <section className="grid grid-cols-2 gap-3 lg:grid-cols-6"><Metric label="총 거래" value={`${analytics.totalTrades}건`} /><Metric label="승률" value={`${analytics.winRate.toFixed(1)}%`} /><Metric label="평균 수익률" value={`${analytics.averageReturn >= 0 ? "+" : ""}${analytics.averageReturn.toFixed(2)}%`} tone={analytics.averageReturn >= 0 ? "positive" : "negative"} /><Metric label="손익비" value={analytics.payoffRatio.toFixed(2)} /><Metric label="최대 손실" value={`${analytics.maximumSingleLoss.toFixed(2)}%`} tone="negative" /><Metric label="최대 낙폭" value={`${analytics.maximumDrawdown < 0 ? "-" : ""}$${Math.abs(analytics.maximumDrawdown).toFixed(0)}`} tone="negative" /></section>
    ...
    <Tabs defaultValue="performance" className="space-y-5">
      ...
      <TabsContent value="performance" className="space-y-5">
        <section className="grid grid-cols-2 gap-3 lg:grid-cols-6"><Metric label="총 손익" value={`${analytics.totalProfit >= 0 ? "+" : "-"}$${Math.abs(analytics.totalProfit).toFixed(0)}`} tone={analytics.totalProfit >= 0 ? "positive" : "negative"} /><Metric label="평균 이익" value={`+${analytics.averageWin.toFixed(2)}%`} tone="positive" /><Metric label="평균 손실" value={`${analytics.averageLoss.toFixed(2)}%`} tone="negative" /><Metric label="거래당 기대값" value={`${analytics.expectancy >= 0 ? "+" : ""}${analytics.expectancy.toFixed(2)}%`} tone={analytics.expectancy >= 0 ? "positive" : "negative"} /><Metric label="최대 연속 손실" value={`${analytics.maximumLossStreak}건`} /><Metric label="상위 5건 수익 비중" value={`${analytics.topFiveProfitShare.toFixed(0)}%`} /></section>
      ...
      <TabsContent value="ticker"><PerformanceTable title="종목별 성과" rows={analytics.tickerRows} showHolding /></TabsContent>
      ...
      <TabsContent value="market" className="space-y-5">
        ...
        {excludedPatterns > 0 && <p className="text-xs text-muted-foreground">표본 5건 미만 조건 {excludedPatterns}개는 시장환경 카드에서 제외했습니다.</p>}
        <div className="grid gap-5 lg:grid-cols-2"><PatternCard title="연결된 거래에서 관찰된 조건" icon={TrendingUp} rows={profitable} positive /><PatternCard title="손실 거래에서 관찰된 조건" icon={TrendingDown} rows={losing} /></div>
...
function PerformanceTable({ title, rows, showHolding = false }: { title: string; rows: PerformanceRow[]; showHolding?: boolean }) { return <Card><div className="border-b border-border px-5 py-4"><h2 className="font-semibold">{title}</h2></div><div className="overflow-x-auto"><table className="w-full min-w-[720px] text-sm"><thead className="bg-surface-2 text-xs text-muted-foreground"><tr>{["구분", "거래 수", "승률", "평균 수익률", "총 손익", ...(showHolding ? ["평균 보유"] : []), "최대 이익", "최대 손실"].map((label) => <th key={label} className="px-4 py-3 text-right first:text-left">{label}</th>)}</tr></thead><tbody className="divide-y divide-border">{rows.map((row) => <tr key={row.label}><td className="px-4 py-3 font-semibold">{row.label}</td><td className="px-4 py-3 text-right">{row.trades}건</td><td className="px-4 py-3 text-right">{row.winRate.toFixed(1)}%</td><td className={cn("px-4 py-3 text-right font-semibold", row.averageReturn >= 0 ? "text-success" : "text-danger")}>{row.averageReturn >= 0 ? "+" : ""}{row.averageReturn.toFixed(2)}%</td><td className={cn("px-4 py-3 text-right", row.totalProfit >= 0 ? "text-success" : "text-danger")}>{row.totalProfit >= 0 ? "+" : "-"}${Math.abs(row.totalProfit).toFixed(0)}</td>{showHolding && <td className="px-4 py-3 text-right">{row.averageHoldingDays.toFixed(1)}일</td>}<td className="px-4 py-3 text-right text-success">+{row.maximumGain.toFixed(2)}%</td><td className="px-4 py-3 text-right text-danger">{row.maximumLoss.toFixed(2)}%</td></tr>)}</tbody></table></div></Card>; }
