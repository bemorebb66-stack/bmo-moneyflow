...
for (const filename of snapshotFiles) {
  ...
  const body = `
    <main class="mx-auto max-w-3xl px-4 py-10" data-static-page="briefing">
      ...
      <p class="mt-8 text-sm">주의사항 · 출처: Yahoo Finance에서 수집한 미국 시장 장마감 일봉. 거래대금은 매수와 매도를 함께 포함하며 실제 순매수나 자금 유입을 뜻하지 않습니다. 공개 시장 데이터 요약이며 투자 추천이 아닙니다.</p>
    </main>`;
  const schema = {
    "@context": "https://schema.org",
    "@graph": [
